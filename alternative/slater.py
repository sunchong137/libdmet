...
def orthonormalizeBasisPHsymm(b):
    ...
    if np.allclose(ovlp - np.eye(nbasis), 0.):
        return normalizeBasis1(b), [nbasis/2, 0, nbasis/2]
    else:
        blocks = getNonDiagBlocks(ovlp)
        diag = set(range(nbasis)) - set(it.chain.from_iterable(blocks))
        diagO = filter(lambda i: i < nbasis/2, diag)
        diagV = filter(lambda i: i >= nbasis/2, diag)
        b1 = np.zeros_like(b)
        for new_i, i in enumerate(diagO):
            b1[:,:,new_i] = b[:,:,i]
        count = len(diagO)
        for block in blocks:
            bsize = len(block) / 2
            ...
            ew = ew[order]
            ev = ev[:, order]
            b1[:,:,count:count+len(block)] = np.tensordot(np.tensordot(b[:,:,block], ev, axes = (2,0)), np.diag(ew**(-0.5)), axes = (2,0))
            count += len(block)
        for new_i, i in enumerate(diagV):
            b1[:,:,new_i+count] = b[:,:,i]
        return b, [len(diagO), nbasis - len(diagO) - len(diagV), len(diagV)]
...
def __embBasis_phsymm(lattice, rho, **kwargs):
    ...
    for s in range(spin):
        ...
        basis[s], counts[s] = orthonormalizeBasisPHsymm(basis[s])
    if spin == 2 and counts[0] != counts[1]:
        # FIXME this might not be the perfect solution
        log.warning("Number of valence orbitals not equal for two spins")
        counts[0][0] = min(counts[0][0], counts[1][0])
        counts[0][2] = min(counts[0][2], counts[1][2])
        counts[0][1] = basis.shape[3] - counts[0][0] - counts[0][2]
    return basis, counts[0]
