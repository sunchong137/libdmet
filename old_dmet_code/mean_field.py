...
class RScfMfdSolver(RMfdSolver):
  ...
  def build_J(self, rho):
    J = np.zeros((self.ncells, self.nscsites, self.nscsites))
  ...
  def build_K(self, rho):
    K = np.zeros((self.ncells, self.nscsites, self.nscsites))
  ...
  def build_L(self, kappa):
    L = np.zeros((self.ncells, self.nscsites, self.nscsites))
...
def init_mfd(inp_mfd, inp_ctrl, lattice, ham, lattice_int2e, orbtype):
  restricted = (orbtype == "RHFB")
