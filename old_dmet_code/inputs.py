...
# (lower_bound, upper_bound) for the given value.
  ...
  'HAMILTONIAN': {
      'Name': None,
      'Type': value(default='Hubbard', allow=('Hubbard', 'NonInt', 'Int')),
  ...
  'DMET': {
      'CalcType': value(default='BCS', allow=('BCS',)),
      'OrbType': value(default='UHFB', allow=('UHFB','RHFB')),
      'Filling': value(limits=(0., 1.)),
      'Localize': value(default='Pin', allow=('Pin', 'Tri', None)),
      ...
      'ConvThrVcor': value(default=5e-3, limits=(0,0.1)),
      'ConvThrMu': value(default=1e-3, limits=(0,0.1)),
      'ConvThrRdm': value(default=5e-5, limits=(0,0.1)),
      'InitGuessType': value(default='RAND', allow=('AF', 'RAND', 'MAN', 'PM', 'ZERO')),
          # initial guess for dmet potential 'BCS' is not needed if we define
          # the function to generate initial guess in Type class
          # AF = antiferromagnetic,
          # RAND = random,
          # MAN = user specified,
          # PM = paramagnetic guess
      'InitGuess': None,
      ...
      'InitMu': value(default=0, allow=floatType),
      'DiisThr': value(default=1e-2,limits=(0,0.1)),
      'DiisStart': 4,
      'DiisDim': 4,
      'UseDelta': value(default=True, allow=(True, False)),
      'TransformIntThr': 5e-5,
      'TransformRdmThr': 1e-5,
      'BathInt2e': False,
      'BathVcor': True,
  },
  'FITTING': {
      'Algorithm': value(default="FullRdm", allow=("FullRdm", "CanonicalH", "CanonicalWfn")),
      'TraceStart': 2,
          # start to trace out diagonal dvloc
      'MFD_Mu_Stop': -1,
          # stop changing mu in mean-field
      'EMB_Mu_Stop': -1,
          # stop changing mu in embedding calculation
      'MaxIter': value(default=200, limits=(50, 2000)),
      'Grad': value(default="Numerical", allow = ("Numerical",)),
      'UpdateDelta': value(default=True, allow=(True, False)),
      'Level': value(default="EmbH1", allow=("EmbH1", "EmbFock", "EmbSCF", "LatSCF"))
  },
  'IMPSOLVER': {
      'ImpSolver': value(default="BLOCK_DMRG", allow=("BLOCK_DMRG",)),
      'TmpDir': settings.TmpDir,
      'StoreDir': settings.StoreDir, # place to store restart information
      'RestartDir': None, # place to read restart information
      'Restart': value(default=True, allow=(True, False)),
      'M': value(default=400, limits=(50, 1000)),
      'N_QP': None,
      'nproc': None,
      'node': None,
      'DoubleOcc': True,
  ...
  'MFD': {
      'Ham': value(default='H1', allow=('H1', 'Fock')),
      'SCF': False,
      'MaxIter': 30,
      'ThrRdm': 1e-6,
      'DiisDim': 8,
  ...
  'FORMAT': {
      ...
      'Walltime': None, # seconds
  },
  'CTRL': {
      'Int2eShape': None, # "SC" or "Lat"
      'Fock': None,    # "None", "Store" or "Comp"
      'EffCore': None, # "None" or "Comp"
      'EffEnv': None,  # "None", "Store" or "Comp"
      'BathInt2e': None,
      "BathVcor": None,
      "FitLevel": None,  # "EmbH1", "EmbFock", "EmbHam", "LatHam",
  }
