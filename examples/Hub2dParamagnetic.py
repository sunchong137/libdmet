...
vcor = dmet.VcorLocal(True, False, nscsites)
vcor.assign(np.eye(nscsites).reshape((1, nscsites, nscsites)) * U * Filling)
Mu = U * Filling
...
rho, Mu = dmet.RHartreeFock(Lat, vcor, Filling, Mu)
...
for iter in range(MaxIter):
    ...
    log.result("Vcor =\n%s", vcor_new.get())
    log.result("Mu (guess) = %20.12f", Mu)
    rho, Mu_new = dmet.RHartreeFock(Lat, vcor_new, Filling, Mu)
    if iter >= TraceStart: # we want to avoid spiral increase of vcor and mu
